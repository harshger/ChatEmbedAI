  ...
  return (
    <DashboardLayout>
      <div className="space-y-8" data-testid="ai-settings-page">
        <div>
          <h1 className="font-clash text-3xl font-bold tracking-tight text-[#0A0A0A]">{t.ai_settings.title}</h1>
          <p className="text-sm text-[#4B5563] mt-1">{t.ai_settings.subtitle}</p>
        </div>

        {saved && (
          <div className="border border-green-300 bg-green-50 p-4 flex items-center gap-3" data-testid="save-success">
            <CheckCircle size={18} className="text-green-600" />
            <p className="text-sm font-bold text-green-700">{t.ai_settings.saved}</p>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-0 border border-gray-200">
          <button onClick={() => setConfig(c => ({ ...c, engine: 'claude' }))} className={`p-8 bg-white border border-gray-200 text-left transition-all ${config?.engine === 'claude' ? 'ring-2 ring-[#002FA7] ring-inset' : 'hover:bg-[#F9FAFB]'}`} data-testid="engine-claude">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 className="font-bold text-[#0A0A0A]">{t.ai_settings.claude_title}</h3>
                {config?.engine === 'claude' && <span className="text-xs text-[#002FA7] font-bold">{t.ai_settings.active}</span>}
              </div>
            </div>
            <p className="text-sm text-[#4B5563]">{t.ai_settings.claude_desc}</p>
            <div className="mt-4 flex gap-2">
              <span className="text-xs bg-green-100 text-green-700 px-2 py-1 font-bold">{t.ai_settings.recommended}</span>
              <span className="text-xs bg-[#F9FAFB] text-[#4B5563] px-2 py-1">{t.ai_settings.eu_compliant}</span>
            </div>
          </button>
          ...
          <button onClick={() => setConfig(c => ({ ...c, engine: 'ollama' }))} className={`p-8 bg-white border border-gray-200 text-left transition-all ${config?.engine === 'ollama' ? 'ring-2 ring-[#002FA7] ring-inset' : 'hover:bg-[#F9FAFB]'}`} data-testid="engine-ollama">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 className="font-bold text-[#0A0A0A]">{t.ai_settings.ollama_title}</h3>
                {config?.engine === 'ollama' && <span className="text-xs text-[#002FA7] font-bold">{t.ai_settings.active}</span>}
              </div>
            </div>
            <p className="text-sm text-[#4B5563]">{t.ai_settings.ollama_desc}</p>
            <div className="mt-4 flex gap-2">
              <span className="text-xs bg-[#F9FAFB] text-[#4B5563] px-2 py-1">{t.ai_settings.self_hosted}</span>
              <span className="text-xs bg-[#F9FAFB] text-[#4B5563] px-2 py-1">{t.ai_settings.fully_private}</span>
            </div>
          </button>
        </div>
        ...
        {config?.engine === 'ollama' && (
          <div className="border border-gray-200 bg-white p-8 space-y-6" data-testid="ollama-config">
            <div className="flex items-center gap-3">
              ...
              <h2 className="font-clash text-xl font-bold text-[#0A0A0A]">{t.ai_settings.ollama_config}</h2>
            </div>

            <div className="border border-yellow-300 bg-yellow-50 p-4 flex items-start gap-3">
              <AlertTriangle size={18} className="text-yellow-600 flex-shrink-0 mt-0.5" />
              <div className="text-sm text-yellow-800">
                <p className="font-bold mb-1">{t.ai_settings.ollama_warning_title}</p>
                <p>{t.ai_settings.ollama_warning_desc}</p>
              </div>
            </div>

            <div>
              <Label className="text-xs font-bold uppercase tracking-[0.15em] text-[#4B5563] mb-2 block">{t.ai_settings.ollama_url}</Label>
              ...
              <p className="text-xs text-[#4B5563] mt-1">{t.ai_settings.ollama_url_hint}</p>
            </div>

            <div>
              <Label className="text-xs font-bold uppercase tracking-[0.15em] text-[#4B5563] mb-2 block">{t.ai_settings.model}</Label>
